...
class Graph {
  ...
  initViz() {
    ...
    this.drawGoldMedal()
    this.viz2 = this.drawSVG(
      'viz-2',
      this.width < smallWidth ? 'Venue Altitude (m)' : 'Olypmic Venue Altitude (meters)'
    )
    ...
    this.viz2
      ...
      .attr(
        'd',
        `M${highestYearApex.x} ${highestYearApex.y} L${this.width - margin.right} ${
          highestYearApex.y
        } L${this.width - margin.right} ${this.height - margin.bottom} L${
          highestYearApex.x + this.xScale.step()
        } ${this.height - margin.bottom} Z`
      )
  ...
  drawAltitudes() {
    const venues = Object.keys(this.years)
      .map(k => this.years[k])
    ...
    const altitudeGroups = this.viz2
      .selectAll('g.altitudes')
      .data(venues)
      .enter()
      .append('g')
    ...
    select('.viz').on('mousemove', () => {
      ...
      if (graphX > margin.left && graphX < this.width - margin.right) {
        ...
        hoveredYear =
          this.xDomain[
            bisect(
              this.xDomain.map(d => this.xScale(d)),
              scaledX
            )
          ]
      } else {
  ...
  updateGoldMedal() {
    ...
    const medalGroupTranslate = `translate(${this.xScale(medalDatum.year)}, ${
      this.yScale(medalDatum.time) - ribbonHeight - radius
    })`
    ...
    this.medalGroup
      ...
      .attr(
        'd',
        `M0 ${ribbonHeight} L0 ${ribbonHeight} L0 ${ribbonHeight} L0 ${ribbonHeight} L0 ${ribbonHeight} Z`
      )
      .transition()
      .duration(medalAnimationTime)
  ...
  drawFlag() {
    ...
    flagGroup
      ...
      .attr(
        'd',
        `M0 ${-flagHeight} Q0 ${-flagHeight - flagWaviness}, 0 ${-flagHeight} Q0 ${
          -flagHeight + flagWaviness
        }, 0 ${-flagHeight} L0 0 Q0 0, 0 0 Q0 0, 0 0 Z`
      )
      .transition()
      .attr('transform', `translate(0, ${-poleHeight})`)
      .on('end', () => {
        this.waveFlag(flagGroup)
      })
  }
  ...
  generateFlagPath(cp0, offset, cp1, invert) {
    invert = invert ? -1 : 1
    return `M0 ${-flagHeight} Q${flagWidth * cp0} ${-flagHeight - invert * flagWaviness}, ${
      flagWidth * offset
    } ${-flagHeight} Q${flagWidth * cp1} ${
      -flagHeight + invert * flagWaviness
    }, ${flagWidth} ${-flagHeight} L${flagWidth} 0 Q${flagWidth * cp1} ${invert * flagWaviness}, ${
      flagWidth * offset
    } 0 Q${flagWidth * cp0} ${-(invert * flagWaviness)}, 0 0 Z`
  }
  ...
  drawYeti() {
    this.yeti
      ...
      .attr(
        'transform',
        `translate(${this.xScale('2002') - this.getYetiSize() / 5}, ${this.height * 0.5})`
      )
      .attr('opacity', 1)
      .transition()
      .delay(1500)
      .attr(
        'transform',
        `translate(${this.xScale('2002') - this.getYetiSize()}, ${this.height * 0.5})`
      )
      .attr('opacity', 0)
  }
  ...
  calculateRibbon(pathIdx) {
    ...
    if (pathIdx === 1) {
      return `M${ribbonRoot} ${ribbonHeight} L${
        -medalWidth + ribbonRoot
      } 0 L${-medalWidth} ${ribbonRoot} L0 ${ribbonHeight + ribbonRoot} Z`
    } else if (pathIdx === 2) {
      return `M${-ribbonRoot} ${ribbonHeight} L${
        medalWidth - ribbonRoot
      } 0 L${medalWidth} ${ribbonRoot} L0 ${ribbonHeight + ribbonRoot} Z`
    } else if (pathIdx === 3) {
      return `M0 0 L${-medalWidth + ribbonRoot} 0 L${
        -medalWidth + ribbonRoot + ribbonWidth * ratio
      } ${ribbonWidth} L${medalWidth - ribbonWidth * ratio - ribbonRoot} ${ribbonWidth} L${
        medalWidth - ribbonRoot
      } 0 Z`
    }
  }
